...
use wide::{
    clamp_i64_to_i32, decorrelate_left_side_scratch, decorrelate_left_side_wide,
    decorrelate_mid_side_scratch, decorrelate_mid_side_wide, decorrelate_right_side_scratch,
    decorrelate_right_side_wide, read_subframe_wide,
};
...
pub struct FlacDecoder {
    ...
    validator: Validator,
    buf: AudioBuffer<i32>,
    side_scratch: Vec<i64>,
    main_scratch: Vec<i64>,
}
...
impl FlacDecoder {
    ...
    fn decode_inner(&mut self, packet: &PacketRef<'_>) -> Result<()> {
        ...
        // Only Bitstream reading for subframes.
        {
            ...
            let wide_side = bits_per_sample >= 32;
            let wide_main = bits_per_sample > 32;
            if wide_side || wide_main || bits_per_sample >= 32 {
                self.main_scratch.resize(n_samples, 0);
                self.side_scratch.resize(n_samples, 0);
            }
            ...
            match header.channel_assignment {
                ChannelAssignment::Independant(channels) => {
                    for i in 0..channels as usize {
                        ...
                        if bits_per_sample >= 32 {
                            read_subframe_wide(&mut bs, bits_per_sample, &mut self.main_scratch)?;
                            for (dst, &src) in plane.iter_mut().zip(&self.main_scratch) {
                                *dst = clamp_i64_to_i32(src);
                            }
                        } else if wide_main {
                            read_subframe_wide(&mut bs, bits_per_sample, &mut self.main_scratch)?;
                            for (dst, &src) in plane.iter_mut().zip(&self.main_scratch) {
                            ...
                            read_subframe(&mut bs, bits_per_sample, plane)?;
                        }
                    }
                }
                ChannelAssignment::LeftSide => {
                    ...
                    if wide_main {
                        ...
                        read_subframe_wide(
                            &mut bs,
                            bits_per_sample + 1,
                            &mut self.side_scratch,
                        )?;
                        decorrelate_left_side_scratch(&mut self.main_scratch, &mut self.side_scratch);
                        for (dst, &src) in left.iter_mut().zip(&self.main_scratch) {
                            *dst = clamp_i64_to_i32(src);
                        }
                        for (dst, &src) in side.iter_mut().zip(&self.side_scratch) {
                            *dst = clamp_i64_to_i32(src);
                        }
                    } else if wide_side {
                        ...
                        decorrelate_left_side_wide(left, &self.side_scratch, side);
                    } else {
                        read_subframe(&mut bs, bits_per_sample, left)?;
                ...
                ChannelAssignment::MidSide => {
                    ...
                    if wide_main {
                        ...
                        decorrelate_mid_side_scratch(&mut self.main_scratch, &mut self.side_scratch);
                        for (dst, &src) in mid.iter_mut().zip(&self.main_scratch) {
                            *dst = clamp_i64_to_i32(src);
                        }
                        for (dst, &src) in side.iter_mut().zip(&self.side_scratch) {
                            *dst = clamp_i64_to_i32(src);
                        ...
                        decorrelate_mid_side_wide(mid, &self.side_scratch, side);
                    } else {
                        read_subframe(&mut bs, bits_per_sample, mid)?;
                        read_subframe(&mut bs, bits_per_sample + 1, side)?;
                ...
                ChannelAssignment::RightSide => {
                    ...
                    if wide_main {
                        ...
                        read_subframe_wide(&mut bs, bits_per_sample, &mut self.main_scratch)?;
                        decorrelate_right_side_scratch(&self.main_scratch, &mut self.side_scratch);
                        for (dst, &src) in right.iter_mut().zip(&self.main_scratch) {
                            *dst = clamp_i64_to_i32(src);
                        }
                        for (dst, &src) in side.iter_mut().zip(&self.side_scratch) {
                            *dst = clamp_i64_to_i32(src);
                        ...
                        decorrelate_right_side_wide(right, &self.side_scratch, side);
                    } else {
                        read_subframe(&mut bs, bits_per_sample + 1, side)?;
                        read_subframe(&mut bs, bits_per_sample, right)?;
...
#[inline(always)]
pub(crate) fn rice_signed_to_i32(word: u32) -> i32 {
