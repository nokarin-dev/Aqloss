...
class PlayerState {
  ...
  Track? get previousTrack =>
      hasPrevious ? queue[queueIndex - 1] : null;

  Track? get nextTrack =>
      hasNext ? queue[queueIndex + 1] : null;
}
...
class PlayerNotifier extends StateNotifier<PlayerState> {
  ...
  void cycleLoopMode() {
    ...
    (state.loopMode.index + 1) % LoopMode.values.length];
  ...
  Future<void> _pollPosition() async {
    ...
    try {
      ...
      if (pos.durationSecs > 0 &&
          pos.positionSecs >= pos.durationSecs - 0.1) {
        _stopTimer();
        await _onTrackEnd();
        return;
      }
    ...
    switch (s.loopMode) {
      ...
      case LoopMode.album:
        final albumTracks = s.queue
            ...
            .toList();
        final idx = albumTracks
            .indexWhere((t) => t.path == s.currentTrack?.path);
